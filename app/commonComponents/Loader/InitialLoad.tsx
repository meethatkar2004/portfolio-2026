...
const InitialLoad = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <div ref={container} id="main-loader-container" className={`${isFixed ? 'fixed inset-0 z-[100]' : 'relative z-0'} w-screen h-screen overflow-hidden`}>
      {/* Intro Black Screen */}
      <div ref={fullScreen} className='absolute inset-0 flex flex-col justify-center items-center bg-background z-10 overflow-hidden'>
      ...
      <div ref={GreenDiv} className='absolute bottom-0 left-0 w-full h-0 bg-primary z-20' />
