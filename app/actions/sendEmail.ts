...
export async function sendEmail(data: SendEmailData) {
  ...
  if (!process.env.RESEND_API_KEY_SECRET) {
    console.error('Missing RESEND_API_KEY_SECRET environment variable');
    return { error: 'Server is misconfigured: Missing email API key.' };
  }
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      headers: {
        ...
        Authorization: `Bearer ${process.env.RESEND_API_KEY_SECRET}`,
      },
      body: JSON.stringify({
        from: 'Portfolio Contact Form <onboarding@resend.dev>',
        to: 'meethatkar2004@gmail.com',
    ...
    if (!res.ok) {
      ...
      return { error: responseData.message || 'Failed to send email via API.' };
    }

    return { success: true, data: responseData };
  } catch (error: unknown) {
    console.error('Resend Action Error (Exception):', error);
    return { error: 'Failed to send email due to a server exception.' };
  }
}
